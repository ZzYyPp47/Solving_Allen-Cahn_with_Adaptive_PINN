...
def test():
    ...
    base = FNN(Arc, func, device)

    point_base = create_point(N_pde,N_bound,N_ini,N_real,device)

    opt_base = torch.optim.Adam(params = base.parameters(), lr=learning_rate)

    pinn_base = pinn('base', total_epochs, tor, loss_func, base, point_base, opt_base, init_method, loss_weight, device)
    ...
    # 加载并测试
    draw(pinn_base,'save/base.pth',device)
    plt.show()
