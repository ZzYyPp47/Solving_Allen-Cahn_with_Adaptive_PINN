...
class create_point(Dataset):
    def __init__(self,N_pde,N_bound,N_ini,N_real,device):
        ...
        # 获取各个部分的长度
        self.len_pde = len(self.pde_x)
        self.len_bound = len(self.bound_x1)
        self.len_ini = len(self.ini_x)
    def rand_point(self,N_pde,N_bound,N_ini,N_real):
        ...
        self.bound_txm1 = torch.rand_like(self.bound_x1, requires_grad=True, dtype=torch.float32,device=self.device)
    ...
    def __getitem__(self,idx):
        ...
        return {
            ...
            'bound': (self.bound_x1[idx_bound], self.bound_tx1[idx_bound], self.bound_xm1[idx_bound], self.bound_txm1[idx_bound]),
