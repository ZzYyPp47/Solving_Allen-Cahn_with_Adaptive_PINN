...
class LossCompute:
    ...
    def loss_pde(self):
        ...
        return self.loss_func(u_t - 0.0001 * u_xx + 5 * (u ** 3) - 5 * u, torch.zeros_like(u))

    def loss_bound(self):
        u_1 = self.model(torch.cat([self.point.bound_x1, self.point.bound_tx1], dim=1))
        u_m1 = self.model(torch.cat([self.point.bound_xm1, self.point.bound_txm1], dim=1))
        u_x_1 = self.gradient(u_1, self.point.bound_x1)
        u_x_m1 = self.gradient(u_m1, self.point.bound_xm1)
        return self.loss_func(u_1, u_m1) + self.loss_func(u_x_1,u_x_m1)

    def loss_ini(self):
        u = self.model(torch.cat([self.point.ini_x, self.point.ini_t], dim=1))
        return self.loss_func(u, (self.point.ini_x ** 2) * torch.cos(torch.pi * self.point.ini_x))
